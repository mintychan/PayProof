...
- **FHEVM smart contracts:** `EncryptedPayroll` for confidential streams, `IncomeOracle` for PoI attestations (powered by `@fhevm/solidity` + Hardhat fhEVM plugin; deploy to Zama Protocol public testnet or fhEVM Coprocessor on Sepolia).
...
- **Demo inputs:** Front-end helpers emit deterministic handles/proofs for local demos; swap them with `@fhevm/sdk` when wiring to live wallets.
...
3. Copy `apps/web/.env.example` → `.env` and provide RPC endpoints and demo addresses (`NEXT_PUBLIC_FHE_RPC_URL`, contract addresses, employer/employee/verifier wallets).
...
- `pnpm test:e2e` – headless Playwright smoke suite (uses `NEXT_PUBLIC_FHE_FALLBACK=1` to generate cryptographically random demo handles when the fhEVM relayer isn’t available).
